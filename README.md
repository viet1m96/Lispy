...
- доступ `lw/sw` только к целым словам;
...
Каждый символ хранится в одном 32-битном слове. Это неэкономично, но очень удобно для лабораторной модели, потому что позволяет работать только через `lw/sw` и не усложнять datapath и компилятор побайтными операциями.
...
- регистры trap-состояния и кодировка системных инструкций уже добавлены как задел на следующую стадию разработки.
...
Текущая ISA классифицируется как:

- **RISC** — фиксированная длина инструкций, арифметика выполняется только над регистрами, доступ к памяти и I/O вынесен в отдельные инструкции;
- **neum** — единое адресное пространство;
- **hw** — hardwired control unit;
- **tick** — модель исполняется с точностью до такта;
- **binary** — машинный код сериализуется в бинарный файл образа.
...
С точки зрения текущей модели:

- `PC` обновляется в конце второго такта;
- ветвления используют `PC_old + off`;
- `jal` пишет `PC + 4` в `rd` и загружает `PC_old + off`;
- `jalr` пишет `PC + 4` в `rd` и загружает `rs1 + off`.

### Детали кодирования

Внутри проекта каждая инструкция представлена enum-моделью, а затем кодируется в 32-битное слово. Для удобства трансляции immediate-поля могут на ранней стадии храниться как символические выражения:
...
Во время ассемблирования эти выражения разрешаются в конкретные числовые значения, после чего инструкция кодируется в бинарный вид.

Это удобно для компилятора, потому что codegen может работать на уровне меток и секций, не вычисляя адреса вручную заранее.
...
- память;
- заготовки под trap-state;
- заготовки под vector-state.
...
Это удобно и для демонстрации лабораторной, и для golden tests, потому что журнал достаточно подробный, но при этом остаётся читаемым.

### Особенности текущей реализации моделирования

#### Fetch

На фазе fetch:
- память читает слово по адресу `PC`;
- считанное значение записывается в `IR`;
- trace фиксирует сам факт выборки.
#### Execute

На фазе execute:
- декодируется содержимое `IR`;
- control unit генерирует набор сигналов;
- datapath применяет их к состоянию машины;
- при необходимости выполняются `lw/sw`, запись регистра и обновление `PC`.

#### Память

Память уже умеет:

- читать из `.text`, `.rodata`, `.data`;
- писать в изменяемую область данных;
- проверять выравнивание word access;
- обслуживать MMIO по словному доступу;
- собирать выходной поток как строку.
